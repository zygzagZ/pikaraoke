...
class TestSpotifyLyricsServiceFetch:
    """Token + lyrics flow uses TOTP-based /api/token (post-2025 Spotify auth).

    Each token mint hits two endpoints in lyrics.py: ``/api/server-time`` then
    ``/api/token``. The TOTP code itself is generated by the singleton in
    ``lyrics_spotify_totp`` and is mocked to keep tests offline.
    """
    ...
    def _server_time_resp(self):
        resp = MagicMock(status_code=200)
        resp.json.return_value = {"serverTime": 1_700_000_000}
        return resp

    def _token_resp(self, *, anonymous=False):
        resp = MagicMock(status_code=200)
    ...
    def _patch_totp(self, code="123456", ver=61):
        totp = MagicMock()
        totp.generate.return_value = (code, ver)
        return patch(
            "pikaraoke.lib.lyrics_spotify_totp.SpotifyTOTP.singleton",
            return_value=totp,
        )
    ...
    def test_token_caches_across_calls(self):
        ...
        with self._patch_totp(), patch(
            "pikaraoke.lib.lyrics.requests.get",
            side_effect=[self._server_time_resp(), self._token_resp()],
        ) as mock_get:
            assert svc._get_spotify_access_token() == "TOK"
            assert svc._get_spotify_access_token() == "TOK"
            # First call mints (server-time + token = 2 requests). Second
            # call hits the in-memory cache, no extra network.
    ...
    def test_token_anonymous_returns_none(self):
        ...
        with self._patch_totp(), patch(
            "pikaraoke.lib.lyrics.requests.get",
            side_effect=[self._server_time_resp(), self._token_resp(anonymous=True)],
        ):
            assert svc._get_spotify_access_token() is None

    def test_token_returns_none_when_totp_unavailable(self):
        from pikaraoke.lib.lyrics_spotify_totp import SpotifyTOTPError

        svc = self._make_service()
        totp = MagicMock()
        totp.generate.side_effect = SpotifyTOTPError("secret repo down")
        with patch(
            "pikaraoke.lib.lyrics_spotify_totp.SpotifyTOTP.singleton",
            return_value=totp,
        ), patch(
            "pikaraoke.lib.lyrics.requests.get",
            side_effect=[self._server_time_resp()],
        ):
            assert svc._get_spotify_access_token() is None

    def test_token_returns_none_when_server_time_fails(self):
        svc = self._make_service()
        bad = MagicMock(status_code=503)
        with self._patch_totp(), patch("pikaraoke.lib.lyrics.requests.get", side_effect=[bad]):
            assert svc._get_spotify_access_token() is None
    ...
    def test_fetch_lrc_via_isrc(self):
        ...
        with self._patch_totp(), patch(
            ...
            # Search call is the third request (server-time, token, search).
            assert "isrc:USRC12345678" in mock_get.call_args_list[2].kwargs["params"]["q"]
    ...
    def test_fetch_lrc_returns_none_on_no_search_match(self):
        ...
        with self._patch_totp(), patch(
            "pikaraoke.lib.lyrics.requests.get",
            side_effect=[self._server_time_resp(), self._token_resp(), search_resp],
        ):
